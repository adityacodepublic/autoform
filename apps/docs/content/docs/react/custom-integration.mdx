...
`FieldWrapper`, `ObjectWrapper`, `ArrayWrapper`, and `ArrayElementWrapper` can later be overridden per field with `fieldConfig({ fieldWrapper, objectWrapper, arrayWrapper, arrayElementWrapper })`, so keep their props compatible with the exported wrapper prop types.
...
`inputProps` contains props from `fieldConfig.inputProps`. Import the `useField` hook from `@acp-autoform/react` to connect the component to the form. `useField({ name: id })` returns exactly what `useController({ name: id })` returns (it is an alias): `field`, `fieldState`, and `formState`. This approach avoids adding `react-hook-form` directly to your package's `dependencies`.
