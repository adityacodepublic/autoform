...
<ModeTab command="@dual-autoform/react @tanstack/react-form" />
...
<ModeTab
  mode="cli"
  command="shadcn@latest add https://raw.githubusercontent.com/adityacodepublic/autoform/refs/heads/tanstack-form-integration/packages/shadcn/registry/autoform-tanstack.json"
/>
...
```tsx
import { AutoForm } from "@dual-autoform/mantine/tanstack-form";
```
...
```tsx
import { AutoForm } from "@dual-autoform/ant/tanstack-form";
```
...
When creating custom field components for the TanStack adapter, use `useFieldContext` from `@dual-autoform/react/tanstack-form`. See [customization](/docs/tanstack/customization#connecting-custom-fields-to-tanstack-form).
