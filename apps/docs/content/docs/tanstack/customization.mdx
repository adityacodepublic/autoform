...
`fieldConfig` helps you customize individual form fields. Use it to set labels, descriptions, input props, field ordering, field-type overrides, and per-field wrappers. See the [`fieldConfig` API](/docs/tanstack/api#fieldconfig) for available props.
...
import type { FieldTypes } from "@dual-autoform/mui/tanstack-form";
...
Keep form components focused on wiring the input. Most form components should not render the `label` or `error` directly because `FieldWrapper` already handles that. Still [`AutoFormFieldProps`](/docs/tanstack/api#custom-fields) includes them for advanced UI integrations.
...
For more examples on creating custom form components see [examples.](/docs/tanstack/faq-examples)
...
<ModeTab
  mode="cli"
  command="shadcn@latest add https://raw.githubusercontent.com/adityacodepublic/autoform/refs/heads/tanstack-form-integration/packages/shadcn/registry/autoform-react-source-tanstack.json"
/>
