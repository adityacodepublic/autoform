...
The `@acp-autoform/react` package exposes a base `<AutoForm>` component that handles all the schema parsing, react-hook-form wiring, and recursive rendering. Your job is simply to provide the **UI Components** (wrappers) and **Form Components** (inputs) to style it.

## 1. Setup

Create a file for your custom AutoForm component (e.g., `MyAutoForm.tsx`). You need to import the base component and types from `@acp-autoform/react`.
...
Import the `useField` hook from `@acp-autoform/react` to connect your components to the form state. `useField` is simply an alias for `useController` from `react-hook-form`, which allows UI integrations to avoid having `react-hook-form` as a direct dependency.
...
Now you can use this `AutoForm` throughout your app just like the official ones!
