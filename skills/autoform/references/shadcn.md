...
The shadcn AutoForm wraps `BaseAutoForm` from `@acp-autoform/react`, providing shadcn-styled `uiComponents` and `formComponents`. User-provided overrides via props are merged on top.
...
```
autoform/
├── index.ts               # Re-exports AutoForm and types
├── AutoForm.tsx            # Main component (wraps BaseAutoForm)
├── types.ts                # AutoFormProps type alias
└── components/
